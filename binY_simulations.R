...
sim.size = function(tau, pi, size = 0.05)
{
  ...
  for(i in 1:m)
  {
    #w/o confounders
    obj1 = lm(M[i,] ~ -1 + X )
    obj2 = glm(Y[i,] ~ -1 + M[i,] + X, family = binomial)
...
write.csv(sim.res, paste0("m",m,"n",n,"sparse_binY.csv"))
sim.res
...
write.csv(sim.res, paste0("m",m,"n",n,"dense_binY.csv"))
beepr::beep(4)
sim.res
